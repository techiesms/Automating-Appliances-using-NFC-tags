...
{
  int pinValue = param.asInt(); // assigning incoming value from pin V4 to a variable
  Serial.print("V5 - "); Serial.println(pinValue);
  switch (pinValue) {
    case 1: // Shooting Mode
      digitalWrite(R1, HIGH); // TubeLights Off
      digitalWrite(R2, LOW);  // Studio Lights On
      digitalWrite(R3, HIGH); // Speakers Off
      digitalWrite(R4, HIGH); // Alexa Off
      break;
    case 2: // Editing Mode
      digitalWrite(R1, LOW); // Tubelight On
      digitalWrite(R2, HIGH); // Studio Lights Off
      digitalWrite(R3, LOW);  // Speakers On
      digitalWrite(R4, LOW); // Alexa On
      break;
    default:
      break;
  }
